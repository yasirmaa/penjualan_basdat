...
// ===== Tampil yang di edit
$id_barang = $_GET['edit_barang'];

$result = pg_query($db, "SELECT * FROM barang INNER JOIN pemasok ON barang.id_pemasok = pemasok.id_pemasok WHERE id_barang = $id_barang");
$edit_barang = pg_fetch_assoc($result);
            ...
            <div class="container main-content p-5">
                <div class="header p-4 pe-5 ps-5 shadow mb-4 w-50 bg-light rounded-3">
                    ...
                    <form action="" method="POST" enctype="multipart/form-data" class="w-100">
                        <input type="hidden" class="form-control" name="id_barang" value="<?= $edit_barang['id_barang'] ?>">
                        <div class="mb-3 row align-items-center">
                            <label for="kode_barang" class="col-sm-3 col-form-label">Kode Barang</label>
                            <div class="col-sm-9">
                                <input type="text" class="form-control" id="kode_barang" name="kode_barang" required value="<?= $edit_barang['kode_barang'] ?> ">
                            </div>
                        </div>
                        <div class="mb-3 row align-items-center">
                            <label for="nama" class="col-sm-3 col-form-label">Nama Barang</label>
                            <div class="col-sm-9">
                                <input type="text" class="form-control" id="nama" name="nama_barang" required value="<?= $edit_barang['nama_barang'] ?> ">
                            </div>
                        </div>
                        <div class="mb-3 row align-items-center">
                            <label for="harga" class="col-sm-3 col-form-label">Harga Satuan</label>
                            <div class="col-sm-9">
                                <input type="number" class="form-control" id="harga" name="harga" required value="<?= $edit_barang['harga'] ?>">
                            </div>
                        </div>
                        <div class="mb-3 row align-items-center">
                            <label for="stok" class="col-sm-3 col-form-label">Jumlah Barang</label>
                            <div class="col-sm-9">
                                <input type="number" class="form-control" id="stok" name="stok" required value="<?= $edit_barang['stok'] ?>">
                            </div>
                        </div>
                        <div class="mb-3 row align-items-center">
                            <label for="inputGroupSelect01" class="col-sm-3 col-form-label">Pemasok</label>
                            <div class="col-sm-9">
                                <select class="form-select" id="inputGroupSelect01" name="id_pemasok" disabled>
                                    <option value="<?= $edit_barang['id_pemasok'] ?>" selected><?= $edit_barang['nama_pemasok'] ?></option>
                                </select>
                            </div>
                        </div>
