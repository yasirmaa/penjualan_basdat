...
$Pemabayran = [];
while ($pemabayran = pg_fetch_assoc($result)) {
    $Pemabayran[] = $pemabayran;
}
...
if (isset($_GET['hapus_pembayaran'])) {
    ...
    $result = pg_query($db, "DELETE FROM pembayaran WHERE id_pembayaran = $id_pembayaran");
    if ($result) {
        echo "
          <script> 
        ...
        echo "
          <script> 
              alert ('Data gagal dihapus karena masih melakukan pembayaran');
              document.location.href = 'pembayaran.php';
          </script>
          ";
    }
        ...
        <div class="main-content w-100">
            ...
            <div class="container main-content p-5">
                <div class="header p-4 pe-5 ps-5 shadow mb-4 w-75 bg-light rounded-3">
                    <h5 class="fw-bold mb-4">Form Pembayaran</h5>
                    <form action="" method="POST" enctype="multipart/form-data" class="w-100">
                        <div class="mb-3 row align-items-center">
                            ...
                            <div class="col-sm-9">
                                <select class="form-select" id="kode_transaksi" name="kode_transaksi">
                        ...
                        <div class="mb-3 row align-items-center">
                            <label for="tgl_bayar" class="col-sm-3 col-form-label">Tanggal Pembayaran</label>
                            <div class="col-sm-9">
                                <input type="date" class="form-control" id="tgl_bayar" name="tgl_bayar" required>
                            </div>
                        </div>
                        <div class="col-12 mt-4 d-flex justify-content-end">
                            <input type="submit" name="tambah" value="Simpan" class="btn btn-primary ps-4 pe-4 ">
                        </div>
                    </form>
                </div>
                <div class="daftar-mhs p-4 shadow bg-light rounded-3">
                    ...
                    <div class="table-responsive bg-body mt-3 mb-4 rounded-3 shadow-sm">
                        <table class="table align-middle">
                            ...
                            <tbody>
                                <?php $i = 1; ?>
                                <?php foreach ($Pemabayran as $pembayaran) : ?>
                                    <tr>
                                        <td class="text-center"><?= $i++ ?>.</td>
                                        <td class="text-center"><?= $pembayaran['kode_transaksi'] ?></td>
                                        <td><?= $pembayaran['nama_pembeli'] ?></td>
                                        <td><?= $pembayaran['nama_barang'] ?></td>
                                        <td class="text-center"><?= $pembayaran['kuantitas'] ?></td>
                                        <!-- <td class="text-center"><?= $pembayaran['total_harga'] ?></td> -->
                                        <td class="text-center"><?= $pembayaran['tgl_transaksi'] ?></td>
                                        <td class="text-center"><?= $pembayaran['metode'] ?></td>
                                        <td class="text-center"><?= $pembayaran['tgl_bayar'] ?></td>
                                        <td class="text-center"><?= $pembayaran['total_bayar'] ?></td>
                                        <td class="text-center">
                                            <a href="pembayaran-edit.php?edit_pembayaran=<?= $pembayaran['id_pembayaran'] ?>">
                                                <button type="button" class="btn btn-warning">
                                                    <i class="fa-regular fa-pen-to-square"></i>
                                                </button>
                                            </a>
                                            <a href="pembayaran.php?hapus_pembayaran=<?= $pembayaran['id_pembayaran'] ?>" onclick="return confirm('Ingin Menghapus Data Tersebut?');">
