...
$result = pg_query($db, "SELECT * FROM pembeli");
while ($pembeli = pg_fetch_assoc($result)) {
    $Pembeli[] = $pembeli;
}

// ===== Tampil yang di edit
$id_pembeli = $_GET['edit_pembeli'];

$result = pg_query($db, "SELECT * FROM pembeli WHERE id_pembeli = $id_pembeli");
$edit_pembeli = pg_fetch_assoc($result);

// ===== Edit pembeli
if (isset($_POST['edit'])) {
    if (edit_pelanggan($_POST)) {
        echo "
          <script> 
        ...
        <div class="main-content w-100">
            ...
            <div class="container main-content p-5">
                <div class="header p-4 pe-5 ps-5 shadow mb-4 w-50 bg-light rounded-3">
                    <h5 class="fw-bold mb-4">Edit Pelanggan</h5>
                    <form action="" method="POST" enctype="multipart/form-data" class="w-100">
                        <input type="hidden" class="form-control" name="id_pembeli" value="<?= $edit_pembeli['id_pembeli'] ?>">
                        <div class="mb-3 row align-items-center">
                            <label for="nama" class="col-sm-3 col-form-label">Nama</label>
                            <div class="col-sm-9">
                                <input type="text" class="form-control" id="nama" name="nama_pembeli" required value="<?= $edit_pembeli['nama_pembeli'] ?>">
                            </div>
                        </div>
                        <div class="mb-3 row align-items-center">
                            ...
                            <div class="col-sm-9 d-flex">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="jenis_kelamin" value="Laki-Laki" required <?= ($edit_pembeli['jenis_kelamin'] == 'Laki-Laki') ? "checked" : "" ?>>
                                    <label class="form-check-label" for="jk_l">
                                        Laki-Laki
                                    </label>
                                </div>
                                <div class="ms-4 form-check">
                                    <input class="form-check-input" type="radio" name="jenis_kelamin" value="Perempuan" <?= ($edit_pembeli['jenis_kelamin'] == 'Perempuan') ? "checked" : "" ?>>
                        ...
                        <div class="mb-3 row align-items-center">
                            ...
                            <div class="col-sm-9">
                                <input type="text" class="form-control" id="no_telp" name="no_telp" required value="<?= $edit_pembeli['no_telp'] ?>">
                            </div>
                        </div>
                        <div class="mb-3 row align-items-center">
                            <label for="alamat" class="col-sm-3 col-form-label">Alamat</label>
                            <div class="col-sm-9">
                                <input type="text" class="form-control" id="alamat" name="alamat" required value="<?= $edit_pembeli['alamat'] ?>">
                            </div>
                        </div>
